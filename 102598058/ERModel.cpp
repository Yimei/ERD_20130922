...
void ERModel::addConnectionFromFile(vector<string> connectionVectorFromFile)
{
	...
	cout << "id = " << connectionVectorFromFile[1] << endl;
...
//vector<string> ERModel::splitPrimaryKey(string line){
//	
...
void ERModel::displayConnectionTable()
{
	...
	for (int i = 0; i < _components.size();i++)
	{
		if (_components[i]->getType()=="C")//是Connector
		{
			cout << setw(3) << _components[i]->getID() << "        |   " << _components[i]->getConnections()[0]->getID() << "  |  " << _components[i]->getConnections()[1]->getID() << endl;
		}
	}
	cout << "------------------------------------" << endl;
}
